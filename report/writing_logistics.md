...
| OpenEvolve-selected `MoP-V2` router | `MoP-V1.3`, `mop_sticky_margin_pct` | public name for the OpenEvolve-tuned `MoP-V1` family |
| disabled prefetching | `Baseline`, `no-prefetch`, `nopref`, `gm_vs_nopref` | universal `1.0x` performance baseline |
| max-prefetcher cap | `pair-best`, `pair_best`, `gm_vs_pair_best` | per-trace `max(MLOP, SPP+PPF)` |
...
| --- | --- | --- |
| Router geomean | `report/figures/stage2_pre_post_geomean.png` | disabled-prefetching baseline at `1.0x`, manual router, OpenEvolve router, max-prefetcher cap |
| Heldout trace profile | `report/figures/stage2_heldout_trace_profile.png` | per-trace speedup for `MLOP`, `SPP+PPF`, and public `MoP-V2` |
...
All current performance plots use disabled prefetching as the `1.0x` baseline. The max-prefetcher result appears as a cap or reference line when shown.

## Number Formatting Rule

Presentation and Markdown prose/tables round displayed performance speedups to at most three decimal places, such as `1.033x` or `0.998x`. Comparisons to the per-trace max prefetcher use `xx.x% of max`. Raw JSON, CSV, and ledger artifacts keep full precision for reproducibility.

## Plot Color Rule

Plots use a stable semantic palette. OpenEvolve router results use `#dc267f`. Manual router results use `#fe6100`. Constituent prefetchers use `#648fff` for `MLOP` and `#785ef0` for `SPP+PPF`. The max-prefetcher cap uses `#ffb000`. Disabled prefetching uses black. Light grey is reserved for gridlines and range guides. Model-search plots use the OpenEvolve-adjacent family `#dc267f`, `#785ef0`, and `#648fff` because all three curves are generated by OpenEvolve runs.
...
- The public report says "max-prefetcher cap", while some raw metric keys say `pair_best`.
