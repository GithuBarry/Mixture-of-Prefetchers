...
void MP3BRepl::initialize_replacement() {
  ...
  for (int i = 0; i < params->dan_predictor_tables; i++) {
    ...
    case F_LM:
    case F_INS:
      if (params->specs[i].xorpc == 0 || params->specs[i].xorpc == 2) {
        trace_bits += 1;
      } else {
    ...
    case F_OFF:
      if (params->specs[i].xorpc == 0 || params->specs[i].xorpc == 2) {
        trace_bits += (params->specs[i].end - params->specs[i].begin);
      } else {
        trace_bits += params->dan_predictor_index_bits;
      }
      break;
  ...
  int sampler_set_bits = params->dan_sampler_assoc * (4                                           // lru stack position
                                                      + params->dan_sampler_tag_bits + trace_bits // bits for the trace stored in each
                                                                                                  // sampler entry
                                                      + 9                                         // confidence bits
                                                     );
  ...
  int max_end = 0;
  for (int i = 0; i < params->dan_predictor_tables; i++) {
    if (params->specs[i].type == F_PC) {
      if (params->specs[i].end > max_end) {
        max_end = params->specs[i].end;
      }
    }
  }
...
void MP3BRepl::update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type,
                                        uint8_t hit) {
...
void MP3BRepl::update_plru_mdpp(int set, int32_t wayID, bool hit, int *vector, uint32_t accessType, bool really, int conf) {
  ...
  assert(vector);
  if (!hit) {
    if (conf >= params->plv[2][0]) {
      newidx = params->plv[2][1];
    } else if (conf >= params->plv[1][0]) {
      newidx = params->plv[1][1];
    } else if (conf >= params->plv[0][0]) {
      newidx = params->plv[0][1];
    }
  } else {
  ...
  if (really) {
    memcpy(plru_bits[set], P, LLC_WAY - 1);
  }
}
...
void MP3BRepl::UpdateSampler(uint32_t setIndex, uint64_t tag, uint32_t tid, uint64_t PC, int32_t way, bool hit, uint32_t accessType, uint64_t paddr) {
  ...
  // determine if this is a sampler set
  {
    ...
    // do a fake promotion to see if this was a burst so we can get a
    // prediction.
    ...
    if (params->dan_use_rrip) {
      ...
      if (!hit) {
        ...
        if (conf >= params->plv[2][0]) {
          position = params->plv[2][1];
        } else if (conf >= params->plv[1][0]) {
          position = params->plv[1][1];
        } else if (conf >= params->plv[0][0]) {
          position = params->plv[0][1];
        }
      } else {
...
stuff:
  ...
  bool record = false;
  if (accessType == LOAD) {
    if (params->dan_record_types & 1) {
      record = true;
    }
  }
  if (accessType == RFO) {
    if (params->dan_record_types & 2) {
      record = true;
    }
  }
  if (accessType == WRITEBACK) {
    if (params->dan_record_types & 8) {
      record = true;
    }
  }
  if (accessType == PREFETCH) {
    if (params->dan_record_types & 16) {
      record = true;
    }
  }
  if (record) {
