...
static feature_spec default_single_1_specs[] = {
    {F_OFF, 15, 1, 6, 0, 1},  {F_PC, 7, 14, 43, 11, 0}, {F_PC, 16, 3, 11, 16, 1}, {F_INS, 16, 0, 0, 0, 1},    {F_OFF, 10, 0, 6, 0, 1}, {F_PC, 10, 1, 53, 10, 0},
    {F_BIAS, 16, 0, 0, 0, 0}, {F_INS, 8, 0, 0, 0, 1},   {F_PC, 17, 6, 20, 0, 1},  {F_BURST, 6, 11, 22, 9, 0}, {F_LM, 9, 0, 0, 0, 0},   {F_PC, 17, 6, 20, 0, 1},
    {F_INS, 16, 2, 47, 2, 0}, {F_INS, 17, 0, 0, 0, 1},  {F_PC, 16, 8, 16, 5, 0},  {F_PC, 17, 6, 20, 14, 1},
};

// this is used in multi-core
static feature_spec default_multi_3_specs[] = {
    {F_BIAS, 1, 0, 0, 0, 0},  {F_PC, 16, 9, 25, 9, 1}, {F_INS, 8, 4, 8, 7, 2},  {F_PC, 6, 9, 28, 12, 1}, {F_OFF, 14, 1, 4, 0, 3},  {F_LM, 7, 7, 51, 3, 1},
    {F_PC, 10, 1, 54, 13, 3}, {F_PC, 10, 3, 32, 5, 1}, {F_PC, 14, 5, 24, 0, 1}, {F_OFF, 13, 4, 4, 0, 2}, {F_TAG, 8, 4, 47, 11, 2}, {F_TAG, 2, 24, 32, 0, 1},
    {F_PC, 12, 10, 30, 0, 1}, {F_PC, 12, 9, 28, 0, 2}, {F_PC, 12, 5, 31, 2, 2}, {F_TAG, 8, 10, 8, 7, 1},
    // # 23.392479
};
...
void mp3b_params::set_parameters(uint32_t num_cpus) {
  if (num_cpus == 1) {
    ...
    dan_rrip_place_position = 2, dan_init_weight = 1;
    dan_dt1 = 27;
    dan_dt2 = 229;
    dan_leaders = 34;
    dan_ignore_prefetch = 1;
...
void sdbp_sampler::access(uint32_t tid, int set, int real_set, uint64_t tag, uint64_t PC, int accessType, uint64_t paddr) {
  ...
  for (i = 0; i < params->dan_sampler_assoc; i++) {
  ...
  if (i == params->dan_sampler_assoc) {
    ...
    if (i == params->dan_sampler_assoc) {
      ...
      for (j = 0; j < params->dan_sampler_assoc; j++) {
        if (blocks[j].lru_stack_position == (unsigned int)(params->dan_sampler_assoc - 1)) {
          break;
        }
      }
...
void sdbp_sampler::make_trace(uint32_t tid, perceptron_predictor *pred, uint32_t setIndex, uint64_t PC, uint32_t tag, int accessType, bool burst,
  ...
  for (int i = 0; i < params->dan_predictor_tables; i++) {
    ...
    if (f->xorpc & 2) {
      trace_buffer[i] ^= 2 * (accessType == PREFETCH);
    }
  }
}
...
sdbp_sampler::sdbp_sampler(mp3b_params *_params, int nsets, int assoc) : params(_params) {
  ...
  // per-core arrays of recent PCs (we count the bits later when we know the max
  // PC width)
  ...
  if (nsampler_sets > nsets) {
    nsampler_sets = nsets;
    fprintf(stderr,
            "warning: number of sampler sets exceeds number of real sets, "
            "setting nsampler_sets to %d\n",
            nsampler_sets);
    fflush(stderr);
  }
  ...
  for (int index = 0; index < nsampler_sets; ++index) {
    sets[index] = sdbp_sampler_set(params);
  }
}
...
perceptron_predictor::perceptron_predictor(mp3b_params *_params, sdbp_sampler *samp) : params(_params), parent(samp) {
  ...
  for (int i = 0; i < params->dan_predictor_tables; i++) {
    ...
    case F_LM:
    case F_INS:
      if (params->specs[i].xorpc == 0) {
        table_entries = 2;
      } else if (params->specs[i].xorpc == 2) {
        table_entries = 4;
      } else {
      ...
      break;
    case F_OFF:
      if (params->specs[i].xorpc == false) {
        table_entries = 1 << (params->specs[i].end - params->specs[i].begin);
      } else {
        table_entries = params->dan_predictor_table_entries;
      }
      break;
    ...
    for (int j = 0; j < table_entries; j++) {
      tables[i][j] = params->dan_init_weight;
    }
...
void perceptron_predictor::block_is_dead(uint32_t tid, sdbp_sampler_entry *block, unsigned int *trace_buffer, bool d, int conf, int pos) {
  ...
  bool do_train = false;
  if (conf < 0) {
    if (conf > -params->dan_theta2) {
      do_train = true;
    }
  } else {
    if (conf < params->dan_theta) {
      do_train = true;
    }
  }
  if (!correct) {
  ...
  for (int i = 0; i < params->dan_predictor_tables; i++) {
    ...
    // for this feature
    if (d) {
      if (params->specs[i].assoc != pos) {
        continue;
      }
    }

    // for a "live" block, only train if it would have been a hit not a
    // placement
    if (!d) {
      if (params->specs[i].assoc <= pos) {
        continue;
    ...
    // int *c = &tables[i][trace_buffer[i] &
    // ((1<<params->dan_predictor_index_bits)-1)];
    int *c = &tables[i][trace_buffer[i] % table_sizes[i]];

    // if the block is dead, increment the counter
    if (d) {
      if (*c < params->dan_counter_max) {
        (*c)++;
      }
    } else {
      if (*c > params->dan_counter_min) {
        (*c)--;
      }
    }
  }
...
int perceptron_predictor::get_prediction(uint32_t tid, int set) {
  ...
  for (int i = 0; i < params->dan_predictor_tables; i++) {
    ...
    // int val = tables[i][trace_buffer[i] &
    // ((1<<params->dan_predictor_index_bits)-1)];
