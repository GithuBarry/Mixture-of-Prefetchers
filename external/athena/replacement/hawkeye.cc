...
void HawkeyeRepl::update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type,
                                           uint8_t hit) {
  stats.update_repl_state.called++;

  // call OPTgen to check whether this access would have receieved hit in OPT
  // policy
  ...
  if (prediction == true) // cache-friendly
  {
    ...
    if (hit) {
      ...
      for (uint32_t index = 0; index < LLC_WAY; ++index) {
        rrip[set][index]++;
        if (rrip[set][index] > knob::hawkeye_max_rrip) {
          rrip[set][index] = knob::hawkeye_max_rrip;
        }
      }
      rrip[set][way] = 0;
    }
...
bool HawkeyePred::predict(uint64_t ip) {
  ...
  uint32_t index = gen_index(ip);
  if (confidence[index]->val() > confidence[index]->max() / 2) // predict
                                                               // cache-friendly
  {
