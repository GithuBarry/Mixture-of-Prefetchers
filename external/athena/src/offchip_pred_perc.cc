...
OffchipPredPerc::OffchipPredPerc(uint32_t _cpu, string _type, uint64_t _seed) : OffchipPredBase(_cpu, _type, _seed) {
  perc_pred = new perceptron_pred_t(knob::ocp_perc_activated_features, knob::ocp_perc_weight_array_sizes, knob::ocp_perc_feature_hash_types,
                                    knob::ocp_perc_activation_threshold, knob::ocp_perc_max_weight, knob::ocp_perc_min_weight, knob::ocp_perc_pos_weight_delta,
                                    knob::ocp_perc_neg_weight_delta, knob::ocp_perc_pos_train_thresh, knob::ocp_perc_neg_train_thresh);
...
void OffchipPredPerc::lookup_address(uint64_t vaddr, uint64_t vpage, uint32_t voffset, bool &first_access) {
  ...
  auto it = find_if(m_page_buffer[set].begin(), m_page_buffer[set].end(), [vpage](ocp_perc_page_buf_entry_t *entry) { return entry->page == vpage; });
...
void OffchipPredPerc::check_and_update_act_thresh() {
  ...
  if (dram_bw >= knob::ocp_perc_high_critical_dram_bw_level && precision <= knob::ocp_perc_poor_precision_thresh) {
    ...
    MYLOG(warmup_complete[cpu],
          ...
          "new_act_thresh: %f",
          +dram_bw, true_pos, false_pos, false_neg, true_neg, precision, recall, old_thresh, new_thresh);
