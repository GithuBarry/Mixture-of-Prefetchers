...
string MemControllerScheduleTypeString[] = {"FR_FCFS", "FRONTAL_FR_FCFS", "FR_FRONTAL_FCFS", "ROB_PART_FR_FCFS", "FR_ROB_PART_FCFS", "CR_FCFS"};
...
void MEMORY_CONTROLLER::reset_remain_requests(PACKET_QUEUE *queue, uint32_t channel) {
  for (uint32_t i = 0; i < queue->SIZE; i++) {
    if (queue->entry[i].scheduled) {
      ...
      uint32_t op_cpu = queue->entry[i].cpu, op_channel = dram_get_channel(op_addr), op_rank = dram_get_rank(op_addr), op_bank = dram_get_bank(op_addr),
      ...
      DP(if (warmup_complete[op_cpu]) {
        cout << queue->NAME << " instr_id: " << queue->entry[i].instr_id << " swrites: " << scheduled_writes[channel] << " sreads: " << scheduled_reads[channel]
             << endl;
      });
    }
  }
...
void MEMORY_CONTROLLER::operate() {
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    ...
    if ((write_mode[i] == 0) &&
        ((WQ[i].occupancy >= DRAM_WRITE_HIGH_WM) || ((RQ[i].occupancy == 0) && (WQ[i].occupancy > 0)))) { // use idle cycles to perform writes
      write_mode[i] = 1;
...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue) {
  ...
  if (index != -1) // scheduler might not find anything if all requests are
                   // already scheduled or all banks are busy
  {
    uint64_t op_addr = queue->entry[index].address;
    uint32_t op_cpu = queue->entry[index].cpu, op_rob_pos = queue->entry[index].rob_position, op_channel = dram_get_channel(op_addr),
             op_rank = dram_get_rank(op_addr), op_bank = dram_get_bank(op_addr), op_row = dram_get_row(op_addr);
#ifdef DEBUG_PRINT
    uint32_t op_column = dram_get_column(op_addr);
#endif
    ...
    // ZLANG:
    og_total_scheduled[queue->entry[index].cpu]++;
    switch (queue->entry[index].type) {
    case LOAD:
      og_demand_scheduled[queue->entry[index].cpu]++;
      break;
    case PREFETCH:
      if (queue->entry[index].fill_level < FILL_DDRP) {
        og_pf_scheduled[queue->entry[index].cpu]++;
      } else {
        og_ocp_scheduled[queue->entry[index].cpu]++;
      }
      break;
    }
...
int MEMORY_CONTROLLER::find_next_request(PACKET_QUEUE *queue, uint8_t &row_buffer_hit) {
  if (queue->is_WQ) {
    ...
    case FR_FCFS:
      return schedule_FR_FCFS(queue, row_buffer_hit);
    case FRONTAL_FR_FCFS:
      return schedule_FRONTAL_FR_FCFS(queue, row_buffer_hit);
    case FR_FRONTAL_FCFS:
      return schedule_FR_FRONTAL_FCFS(queue, row_buffer_hit);
    case ROB_PART_FR_FCFS:
      return schedule_ROB_PART_FR_FCFS(queue, row_buffer_hit);
    case FR_ROB_PART_FCFS:
      return schedule_FR_ROB_PART_FCFS(queue, row_buffer_hit);
    case CR_FCFS:
      return schedule_CR_FCFS(queue, row_buffer_hit);
    default:
      return -1;
    }
  }
...
void MEMORY_CONTROLLER::process(PACKET_QUEUE *queue) {
  ...
  uint32_t op_cpu = queue->entry[request_index].cpu, op_channel = dram_get_channel(op_addr), op_rank = dram_get_rank(op_addr), op_bank = dram_get_bank(op_addr);
#ifdef DEBUG_PRINT
  ...
  if (bank_request[op_channel][op_rank][op_bank].cycle_available <= current_core_cycle[op_cpu]) {
    ...
    if (dbus_cycle_available[op_channel] <= current_core_cycle[op_cpu]) {
      if (queue->is_WQ) {
        ...
        // send data back to the core cache hierarchy, only if the request is
        // not DDRP
        if (queue->entry[request_index].fill_level < FILL_DDRP) {
          ...
          DDRP_DP(if (warmup_complete[op_cpu]) {
            ...
            cout << " full_addr: " << hex << queue->entry[request_index].full_addr << dec << " type: " << +queue->entry[request_index].type
                 << " fill_level: " << queue->entry[request_index].fill_level;
            cout << " current_cycle: " << current_core_cycle[op_cpu] << " event_cycle: " << queue->entry[request_index].event_cycle << endl;
          });
        } else {
      ...
      // cout << "[DEQUEUE_" << queue->NAME << "] " << " id: " <<
      // queue->entry[request_index].id << " cpu: " <<
      // queue->entry[request_index].cpu << " instr_id: " <<
      // queue->entry[request_index].instr_id; cout << " address: " << hex <<
      // queue->entry[request_index].address << " full_addr: " <<
      // queue->entry[request_index].full_addr << dec; cout << " instruction: "
      // << (uint32_t)queue->entry[request_index].instruction << " is_data: " <<
      // (uint32_t)queue->entry[request_index].is_data; cout << " timestamp: "
      // << uncore.cycle; cout << " entry_enq_timestamp: " <<
      // queue->entry[request_index].enque_cycle[queue->module_type][queue->queue_type];
      // cout << " entry_deq_timestamp: " <<
      // queue->entry[request_index].deque_cycle[queue->module_type][queue->queue_type]
      // << endl;
      queue->remove_queue(&queue->entry[request_index], uncore.cycle);
      update_process_cycle(queue);
    } else {
      // data bus is busy, the available bank cycle time is fast-forwarded for
      // faster simulation
...
int MEMORY_CONTROLLER::add_rq(PACKET *packet) {
  ...
  if (all_warmup_complete < NUM_CPUS && return_data_to_core) {
    ...
    DDRP_DP(if (warmup_complete[packet->cpu]) {
      ...
      cout << " full_addr: " << packet->full_addr << dec << " type: " << +packet->type << " fill_level: " << packet->fill_level << " return_data_before_warmup"
           << endl;
    });
  ...
  for (uint32_t index = 0; index < DRAM_RQ_SIZE; index++) {
    if (RQ[channel].entry[index].address == 0) {
      ...
      // packet->instr_id; cout << " address: " << hex << packet->address << "
      // full_addr: " << packet->full_addr << dec; cout << " instruction: " <<
      // (uint32_t)packet->instruction << " is_data: " <<
      // (uint32_t)packet->is_data; cout << " timestamp: " << uncore.cycle; cout
      // << " packet_enq_timestamp: " << packet->enque_cycle[IS_DRAM][IS_RQ] <<
      // " packet_deq_timestamp: " << packet->deque_cycle[IS_DRAM][IS_RQ]; cout
      // << " entry_enq_timestamp: " <<
      // RQ[channel].entry[index].enque_cycle[IS_DRAM][IS_RQ] << "
      // entry_deq_timestamp: " <<
      // RQ[channel].entry[index].deque_cycle[IS_DRAM][IS_RQ] << endl;

#ifdef DEBUG_PRINT
      uint32_t channel = dram_get_channel(packet->address), rank = dram_get_rank(packet->address), bank = dram_get_bank(packet->address),
               row = dram_get_row(packet->address), column = dram_get_column(packet->address);
#endif
      ...
      DDRP_DP(if (warmup_complete[packet->cpu]) {
        cout << "[" << NAME << "_RQ] " << __func__ << " instr_id: " << packet->instr_id << " address: " << hex << packet->address;
        cout << " full_addr: " << packet->full_addr << dec << " type: " << +packet->type << " fill_level: " << packet->fill_level << " ADDED_IN_DRAM_RQ_"
             << channel;
        cout << " occupancy: " << RQ[channel].occupancy << " current: " << current_core_cycle[packet->cpu] << " event: " << packet->event_cycle << endl;
      });
...
int MEMORY_CONTROLLER::add_wq(PACKET *packet) {
  ...
  for (index = 0; index < DRAM_WQ_SIZE; index++) {
    if (WQ[channel].entry[index].address == 0) {
      ...
      // cout << "[ENQUEUE_" << WQ[channel].NAME << "] " << " id: " <<
      // packet->id << " cpu: " << packet->cpu << " instr_id: " <<
      // packet->instr_id; cout << " address: " << hex << packet->address << "
      // full_addr: " << packet->full_addr << dec; cout << " instruction: " <<
      // (uint32_t)packet->instruction << " is_data: " <<
      // (uint32_t)packet->is_data; cout << " timestamp: " << uncore.cycle; cout
      // << " packet_enq_timestamp: " << packet->enque_cycle[IS_DRAM][IS_WQ] <<
      // " packet_deq_timestamp: " << packet->deque_cycle[IS_DRAM][IS_WQ]; cout
      // << " entry_enq_timestamp: " <<
      // WQ[channel].entry[index].enque_cycle[IS_DRAM][IS_WQ] << "
      // entry_deq_timestamp: " <<
      // WQ[channel].entry[index].deque_cycle[IS_DRAM][IS_WQ] << endl;
...
void MEMORY_CONTROLLER::update_schedule_cycle(PACKET_QUEUE *queue) {
  ...
  if (min_index < queue->SIZE) {
    DP(if (warmup_complete[queue->entry[min_index].cpu]) {
      ...
      cout << " event: " << queue->entry[min_index].event_cycle << " current: " << current_core_cycle[queue->entry[min_index].cpu]
...
//             uncore.cycle, uncore.DRAM.rq_enqueue_count,
//             uncore.DRAM.last_enqueue_count, uncore.DRAM.epoch_enqueue_count,
//             uncore.DRAM.bw);
...
uint32_t MEMORY_CONTROLLER::get_bw(uint32_t core_id) {

  // uint64_t this_epoch_enqueue_count = og_enqueue_count - og_last_enqueue_count;
  // og_epoch_enqueue_count = (og_epoch_enqueue_count / 2) + this_epoch_enqueue_count;
  // uint32_t bw = (100.0f * og_epoch_enqueue_count) / DRAM_DBUS_MAX_CAS * 256 / knob::og_instr_epoch_len ;
  // og_last_enqueue_count = og_enqueue_count;
  // return bw;
  return og_bw;
}

uint32_t MEMORY_CONTROLLER::get_overall_bw(uint32_t core_id) {
  uint32_t overall_bw = 0;
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    for (uint32_t j = 0; j < RQ[i].SIZE; j++) {
      if (RQ[i].entry[j].cpu == core_id) {
        overall_bw++;
      }
    }
  }
...
uint32_t MEMORY_CONTROLLER::get_pf_bw(uint32_t core_id) {
  uint32_t pf_bw = 0;
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    for (uint32_t j = 0; j < RQ[i].SIZE; j++) {
      if (RQ[i].entry[j].cpu == core_id && RQ[i].entry[j].type == PREFETCH && RQ[i].entry[j].fill_level != FILL_DDRP) {
        pf_bw++;
      }
    }
  }
...
uint32_t MEMORY_CONTROLLER::get_ocp_bw(uint32_t core_id) {
  uint32_t ocp_bw = 0;
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    for (uint32_t j = 0; j < RQ[i].SIZE; j++) {
      if (RQ[i].entry[j].cpu == core_id && RQ[i].entry[j].fill_level == FILL_DDRP) {
        ocp_bw++;
      }
    }
  }
...
uint32_t MEMORY_CONTROLLER::get_demand_bw(uint32_t core_id) {
  uint32_t demand_bw = 0;
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    for (uint32_t j = 0; j < RQ[i].SIZE; j++) {
      if (RQ[i].entry[j].cpu == core_id && RQ[i].entry[j].type == LOAD) {
        demand_bw++;
      }
    }
  }
...
uint32_t MEMORY_CONTROLLER::get_bw_needed(uint32_t core_id) {
  uint32_t bw_needed = 0;
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) {
    for (uint32_t j = 0; j < RQ[i].SIZE; j++) {
      if (RQ[i].entry[j].cpu == core_id && RQ[i].entry[j].type == LOAD) {
        uint32_t op_channel = dram_get_channel(RQ[i].entry[j].address);
        uint32_t op_rank = dram_get_rank(RQ[i].entry[j].address);
        uint32_t op_bank = dram_get_bank(RQ[i].entry[j].address);
        if (bank_request[op_channel][op_rank][op_bank].working && bank_request[op_channel][op_rank][op_bank].working_type == PREFETCH) {
          bw_needed++;
        }
      }
    }
