...
uint32_t process_FLPPrediction(state_info_t *state, uint64_t metadata, int32_t hash_type, uint32_t weight_array_size) {
  // Use FLP's prediction as a feature
  uint32_t flp_pred = state->flp_prediction;
  uint32_t hashed_val = HashZoo::getHash((uint32_t)hash_type, flp_pred);
  return (hashed_val % weight_array_size);
}

uint32_t perceptron_pred_t::generate_index_from_feature(feature_type_t feature, state_info_t *state, uint64_t metadata, int32_t hash_type,
  ...
  case feature_type_t::PC_FirstAccess:
    return process_PC_FirstAccess(state, metadata, hash_type, weight_array_size);
  case feature_type_t::Offset_FirstAccess:
    return process_Offset_FirstAccess(state, metadata, hash_type, weight_array_size);
  ...
  case feature_type_t::PC_CLWordOffset:
    return process_PC_CLWordOffset(state, metadata, hash_type, weight_array_size);
  ...
  case feature_type_t::PC_CLDWordOffset:
    return process_PC_CLDWordOffset(state, metadata, hash_type, weight_array_size);
  ...
  case feature_type_t::FLPPrediction:
    return process_FLPPrediction(state, metadata, hash_type, weight_array_size);
  default:
    assert(false);
  }
