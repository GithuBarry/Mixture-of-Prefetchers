...
std::string PACKET::to_string() {
  std::stringstream ss;
  ss << "ROB_id: " << rob_index << " ROB_pos: " << rob_position << " instr_id: " << instr_id << " is_ins: " << (uint32_t)instruction
     << " is_data: " << (uint32_t)is_data << " addr: " << hex << address << dec << " full_addr: " << hex << full_addr << dec << " fill_level: " << fill_level;
  return ss.str();
}
...
int8_t PACKET_QUEUE_BASE::get_module_type() {
  if (NAME.find("ITLB") != string::npos) {
    ...
    return IS_L1D;
  } else if (NAME.find("L2C") != string::npos) {
    return IS_L2C;
  } else if (NAME.find("LLC") != string::npos) {
    return IS_LLC;
...
int8_t PACKET_QUEUE_BASE::get_queue_type() {
  if (NAME.find("RQ") != string::npos) {
    return IS_RQ;
  } else if (NAME.find("WQ") != string::npos) {
    return IS_WQ;
  } else if (NAME.find("PQ") != string::npos) {
    return IS_PQ;
  } else if (NAME.find("MSHR") != string::npos) {
    return IS_MSHR;
  } else if (NAME.find("PROCESSED") != string::npos) {
    return IS_PROCESSED;
  }
  return -1;
}
      ...
      if (NAME == "L1D_WQ") {
        if (entry[i].full_addr == packet->full_addr) {
          DP(if (warmup_complete[packet->cpu]) {
            cout << "[" << NAME << "] " << __func__ << " cpu: " << packet->cpu << " instr_id: " << packet->instr_id << " same address: " << hex
                 << packet->address;
...
int PACKET_QUEUE::add_queue(PACKET *packet, uint64_t timestamp) {
  ...
  // cout << "[INSIDE_ENQUEUE_" << NAME << "] " << " id: " << packet->id << "
  // cpu: " << packet->cpu << " instr_id: " << packet->instr_id; cout << "
  // address: " << hex << entry[tail].address << " full_addr: " <<
  // entry[tail].full_addr << dec; cout << " instruction: " <<
  // (uint32_t)packet->instruction << " is_data: " << (uint32_t)packet->is_data;
  // cout << " timestamp: " << timestamp << " packet_timestamp: " <<
  // packet->enque_cycle[module_type][queue_type] << " entry_timestamp: " <<
  // entry[tail].enque_cycle[module_type][queue_type] << endl;
...
void PACKET_QUEUE::remove_queue(PACKET *packet, uint64_t timestamp) {
  ...
  // cout << "[INSIDE_DEQUEUE_" << NAME << "] " << " id: " << packet->id << "
  // cpu: " << packet->cpu << " instr_id: " << packet->instr_id; cout << "
  // address: " << hex << packet->address << " full_addr: " << packet->full_addr
  // << dec; cout << " instruction: " << (uint32_t)packet->instruction << "
  // is_data: " << (uint32_t)packet->is_data; cout << " timestamp: " <<
  // timestamp << " packet_timestamp: " <<
  // packet->deque_cycle[module_type][queue_type] << endl;
...
int PACKET_QUEUE_PRIORITY::add_queue(PACKET *packet, uint64_t timestamp) {
  ...
  if (occupancy == 0) {
    ...
    assert(comp != 0); // enforce a strict ordering. There should NOT be two
                       // entires with same priority
    if (comp > 0) {
      head = (occupancy - 1);
      head_iterator = std::prev(entry.end());
    }
