...
string state_info_t::to_string() {
  ...
  ss << "PC: " << setw(8) << hex << pc << dec << " data_id: " << setw(2) << data_index << " vaddr: " << setw(12) << hex << vaddr << dec
     << " vpage: " << setw(12) << hex << vpage << dec << " voffset: " << setw(2) << voffset << " fa: " << setw(1) << first_access << " vclo: " << setw(2)
     << v_cl_offset << " vclwo: " << setw(2) << v_cl_word_offset << " vcldwo: " << setw(2) << v_cl_dword_offset;

  return ss.str();
}

perceptron_pred_t::perceptron_pred_t(vector<int32_t> _activated_features, vector<int32_t> weight_array_sizes, vector<int32_t> hash_types, float threshold,
                                     float max_w, float min_w, float pos_delta, float neg_delta, float pos_thresh, float neg_thresh)
    : activation_threshold(threshold), max_weight(max_w), min_weight(min_w), pos_weight_delta(pos_delta), neg_weight_delta(neg_delta),
      pos_train_thresh(pos_thresh), neg_train_thresh(neg_thresh) {
...
void perceptron_pred_t::predict(state_info_t *state, bool &prediction, float &perc_weight_sum) {
  ...
  for (uint32_t feature = 0; feature < num_features; ++feature) {
    ...
    cummulative_weight += weights[feature].array[weight_indices[feature]]; // sum up all feature
                                                                           // weights
  }
...
void perceptron_pred_t::train(state_info_t *state, float perc_weight_sum, bool pred_output, bool true_output) {
  ...
  MYLOG(warmup_complete[cpu], "perc_train %s perc_weight_sum: %f offchip_real: %d offchip_pred: %d", state->to_string().c_str(), perc_weight_sum, true_output,
        pred_output);
...
void perceptron_pred_t::incr_weights(vector<uint32_t> weight_indices) {
  for (uint32_t feature = 0; feature < num_features; ++feature) {
    if ((weights[feature].array[weight_indices[feature]] + pos_weight_delta) <= max_weight) {
      MYLOG(warmup_complete[cpu], "feature %s index %d old_weight %f", feature_names[activated_features[feature]].c_str(), weight_indices[feature],
            weights[feature].array[weight_indices[feature]]);
      weights[feature].array[weight_indices[feature]] += pos_weight_delta;
      MYLOG(warmup_complete[cpu], "feature %s index %d new_weight %f", feature_names[activated_features[feature]].c_str(), weight_indices[feature],
            weights[feature].array[weight_indices[feature]]);
      stats.weight.incr_done[activated_features[feature]]++;
    } else {
      MYLOG(warmup_complete[cpu], "feature %s index %d saturated_weight %f", feature_names[activated_features[feature]].c_str(), weight_indices[feature],
            weights[feature].array[weight_indices[feature]]);
...
vector<uint32_t> perceptron_pred_t::generate_indices_from_state(state_info_t *state) {
  ...
  for (uint32_t index = 0; index < num_features; ++index) {
    indices.push_back(
        generate_index_from_feature((feature_type_t)activated_features[index], state, 0xdeadbeef, feature_hash_types[index], weights[index].size));
  }
  return indices;
}
