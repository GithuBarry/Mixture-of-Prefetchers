...
bool OffchipPredTTP::predict(ooo_model_instr *arch_instr, uint32_t data_index, LSQ_ENTRY *lq_entry) {
  ...
  /* tag present in catalog cache means the access will likely hit in cache
   * hierarchy. Otherwise, it will likely go off-chip.
   */
  return found ? false : true;
}

void OffchipPredTTP::update_catalog_cache(uint64_t vaddr, uint64_t paddr, bool went_offchip) {
  /* Catalog cache stores the partial tags of cachelines that did not go
   * off-chip. We can conceptually think this as a 1-bit information per
   * cacheline, as mentioned in the paper.
   */
