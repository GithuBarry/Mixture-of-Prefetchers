...
void O3_CPU::initialize_offchip_predictor(uint64_t seed) {
  ...
  } else if (!knob::offchip_pred_type.compare("tlp")) {
    cout << "Adding Offchip predictor: Two Level Perceptron (TLP)" << endl;
    offchip_pred = (OffchipPredTLP *)new OffchipPredTLP(cpu, knob::offchip_pred_type, seed);
  }
}

void O3_CPU::print_config_offchip_predictor() {
  cout << "offchip_pred_type " << knob::offchip_pred_type << endl << "offchip_pred_mark_merged_load " << knob::offchip_pred_mark_merged_load << endl << endl;
