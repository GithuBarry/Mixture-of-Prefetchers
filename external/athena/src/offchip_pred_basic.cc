...
void OffchipPredBasic::train(ooo_model_instr *arch_instr, uint32_t data_index, LSQ_ENTRY *lq_entry) {
  ...
  MYLOG(warmup_complete[cpu],
        "load_pc: %x count: %d data_idx: %d vaddr: %x vpage: %x voffset: %d "
        "offchip_real: %d offchip_pred: %d table_index: %d total_counter: %d "
        "miss_counter: %d",
        load_pc, count, data_index, vaddr, vpage, vofffset, (uint32_t)lq_entry->went_offchip, (uint32_t)lq_entry->went_offchip_pred, index,
        m_table[index].total.val(), m_table[index].miss.val());
}

uint32_t OffchipPredBasic::get_hash(uint64_t load_pc, uint32_t count, uint32_t data_index, uint64_t vaddr, uint64_t vpage, uint32_t voffset,
                                    bool first_access) {
  switch (knob::ocp_basic_feature_type) {
  case ocp_basic_pc:
    return get_hash_pc(load_pc);
  case ocp_basic_pc_count:
    return get_hash_pc_count(load_pc, count);
  case ocp_basic_pc_data_index:
    return get_hash_pc_data_index(load_pc, data_index);
  case ocp_basic_pc_offset:
    return get_hash_pc_offset(load_pc, voffset);
  case ocp_basic_pc_page:
    return get_hash_pc_page(load_pc, vpage);
  case ocp_basic_pc_addr:
    return get_hash_pc_addr(load_pc, vaddr);
  case ocp_basic_cl_first_access:
    return get_hash_cl_first_access(first_access);
  default:
    assert(false);
  }
}
