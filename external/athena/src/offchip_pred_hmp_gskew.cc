...
void OffchipPredHMPGskew::dump_stats() {
  cout << "ocp_hmp_gskew_predict_called " << stats.predict.called << endl << endl << "ocp_hmp_gskew_train_called " << stats.train.called << endl;
...
confidence_state_t OffchipPredHMPGskew::update_confidence_counter(uint32_t hash_id, confidence_state_t old_conf, bool went_offchip) {
  ...
  if (old_conf == STRONG_NT) {
    ...
    if (new_conf == WEAK_NT) {
      stats.train.strong_nt_to_weak_nt[hash_id]++;
    } else {
      stats.train.strong_nt_to_strong_nt[hash_id]++;
    }
  } else if (old_conf == WEAK_NT) {
    new_conf = went_offchip ? WEAK_T : STRONG_NT;
    if (new_conf == WEAK_T) {
      stats.train.weak_nt_to_weak_t[hash_id]++;
    } else {
      stats.train.weak_nt_to_strong_nt[hash_id]++;
    }
  } else if (old_conf == WEAK_T) {
    new_conf = went_offchip ? STRONG_T : WEAK_NT;
    if (new_conf == STRONG_T) {
      stats.train.weak_t_to_strong_t[hash_id]++;
    } else {
      stats.train.weak_t_to_weak_nt[hash_id]++;
    }
  } else if (old_conf == STRONG_T) {
    new_conf = went_offchip ? STRONG_T : WEAK_T;
    if (new_conf == STRONG_T) {
      stats.train.strong_t_to_strong_t[hash_id]++;
    } else {
      stats.train.strong_t_to_weak_t[hash_id]++;
    }
  } else {
    assert(false);
  }
