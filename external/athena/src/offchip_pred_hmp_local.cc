...
void OffchipPredHMPLocal::train(ooo_model_instr *arch_instr, uint32_t data_index, LSQ_ENTRY *lq_entry) {
  ...
  if (old_conf == STRONG_NT) {
    ...
    if (new_conf == WEAK_NT) {
      stats.train.strong_nt_to_weak_nt++;
    } else {
      stats.train.strong_nt_to_strong_nt++;
    }
  } else if (old_conf == WEAK_NT) {
    new_conf = went_offchip ? WEAK_T : STRONG_NT;
    if (new_conf == WEAK_T) {
      stats.train.weak_nt_to_weak_t++;
    } else {
      stats.train.weak_nt_to_strong_nt++;
    }
  } else if (old_conf == WEAK_T) {
    new_conf = went_offchip ? STRONG_T : WEAK_NT;
    if (new_conf == STRONG_T) {
      stats.train.weak_t_to_strong_t++;
    } else {
      stats.train.weak_t_to_weak_nt++;
    }
  } else if (old_conf == STRONG_T) {
    new_conf = went_offchip ? STRONG_T : WEAK_T;
    if (new_conf == STRONG_T) {
      stats.train.strong_t_to_strong_t++;
    } else {
      stats.train.strong_t_to_weak_t++;
    }
  } else {
  ...
  MYLOG(warmup_complete[cpu],
        "pc %lx lhr_index %d went_offchip %d old_conf %d new_conf %d "
        "old_local_history %x new_local_history %x",
        pc, lhr_index, +went_offchip, old_conf, new_conf, old_local_history, new_local_history);
}
