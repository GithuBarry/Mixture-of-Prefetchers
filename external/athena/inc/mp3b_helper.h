...
class mp3b_params {
public:
  int dan_promotion_threshold = 256, dan_init_weight = 1, dan_dt1 = 55, dan_dt2 = 1024, dan_rrip_place_position = 0, dan_leaders = 34,
      ...
      dan_record_types = 27, // mask for deciding which kinds of memory
                             // accesses to record in history
      ...
      dan_threshold = 8, dan_theta2 = 210, dan_theta = 110,
      // number of partial tag bits kept per sampler entry
      dan_sampler_tag_bits = 16, dan_samplers = 80,
      ...
      dan_counter_min, dan_counter_max;
...
struct sdbp_sampler_entry {
  unsigned int lru_stack_position, tag,
...
struct sdbp_sampler {
  ...
  unsigned int trace_buffer[MAX_PATH_LENGTH + 1]; // trace is built here before
                                                  // prediction
  ...
  void make_trace(uint32_t tid, perceptron_predictor *pred, uint32_t setIndex, uint64_t PC, uint32_t tag, int accessType, bool burst, bool insertion,
                  bool lastmiss, unsigned int offset);
};
