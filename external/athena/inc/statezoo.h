...
uint32_t LearningEngineCMAC2::process_delta(int32_t delta, uint32_t plane) {
  ...
  uint32_t unsigned_delta = (delta < 0) ? (((-1) * delta) + (1 << (DELTA_BITS - 1))) : delta; /* converts into 7 bit signed
                                                                                                 representation */
...
uint32_t LearningEngineCMAC2::get_processed_feature(Feature feature, State *state, uint32_t plane) {
  ...
  switch (feature) {
  case PC:
    return process_PC(pc, plane);
  case Offset:
    return process_offset(offset, plane);
  case Delta:
    return process_delta(delta, plane);
  case PC_path:
    return process_PC_path(pc_path, plane);
  case Offset_path:
    return process_offset_path(offset_path, plane);
  case Delta_path:
    return process_delta_path(delta_path, plane);
  case Address:
    return process_address(address, plane);
  case Page:
    return process_page(page, plane);
  default:
