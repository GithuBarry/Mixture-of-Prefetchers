...
/// hash a block of memory
template <unsigned int Unroll> uint32_t fnv1a_unrolled(const void *data, size_t numBytes, uint32_t hash = Seed) {
#ifndef NDEBUG
...
template <> inline uint32_t fnv1a_unrolled<0>(const void *data, size_t numBytes, uint32_t hash /*= Seed*/) {
  return fnv1a(data, numBytes, hash);
}
/// not unrolled at all
template <> inline uint32_t fnv1a_unrolled<1>(const void *data, size_t numBytes, uint32_t hash /*= Seed*/) {
  return fnv1a(data, numBytes, hash);
}
} // namespace FNV
