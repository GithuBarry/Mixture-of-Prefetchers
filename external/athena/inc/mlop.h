/* Based on Multi-Lookahead Offset Prefetcher (MLOP) - 3rd Data Prefetching
 * Championship */
 ...
 * This version of MLOP does not prefetch into L3 so there are 4 states in total
 * (2-bit states).
 */
...
class AccessMapData {
public:
  /* block states are represented with a `MLOP_State` and an `int` in this
   * software implementation but in a hardware implementation, they'd be
   * represented with only 2 bits. */
...
public:
  ...
  /**
   * Sets specified block to given state. If new state is ACCESS, the block will
   * also be pushed in the zone's queue.
   */
  void set_state(uint64_t block_number, MLOP_State new_state, int new_fill_level = 0) {
    ...
    // else
    //     assert(new_fill_level == FILL_L1 || new_fill_level == FILL_L2 ||
    //     new_fill_level == FILL_LLC);
...
private:
  inline bool is_inside_zone(uint32_t zone_offset) {
    return (0 <= zone_offset && zone_offset < this->blocks_in_zone);
  }
  ...
  /**
   * Contains best offsets for each degree of prefetching. A degree will have
   * several offsets if they all had maximum score (thus, a vector of vectors).
   * A degree won't have any offsets if all best offsets were redundant (already
   * selected in previous degrees).
   */
  vector<vector<int>> pf_offset;

  vector<vector<int>> offset_scores;
  vector<int> pf_level;    /* the prefetching level for each degree of prefetching
                              offsets */
  uint32_t update_cnt = 0; /* tracks the number of score updates, round is over
                              when `update_cnt == NUM_UPDATES` */
