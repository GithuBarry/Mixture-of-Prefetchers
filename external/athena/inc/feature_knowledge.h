...
public:
  ...
  uint32_t getMaxAction(State *state); /* Called by featurewise engine only to
                                          get a consensus from all the features
                                        */
  ...
  inline float get_min_weight() {
    return min_weight;
  }
  inline float get_max_weight() {
    return max_weight;
  }
};
