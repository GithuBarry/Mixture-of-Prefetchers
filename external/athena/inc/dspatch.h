...
enum DSPatch_pref_candidate { PAT_NONE = 0, PAT_COVP, PAT_ACCP, Num_DSPatch_pref_candidates };
...
public:
  ...
  inline void incr(uint32_t max = UINT_MAX) {
    counter = (counter < max ? counter + 1 : counter);
  }
  inline void decr(uint32_t min = 0) {
    counter = (counter > min ? counter - 1 : counter);
  }
