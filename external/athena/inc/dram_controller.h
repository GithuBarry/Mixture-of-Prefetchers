...
public:
  ...
  uint32_t og_total_scheduled[NUM_CPUS], og_pf_scheduled[NUM_CPUS], og_ocp_scheduled[NUM_CPUS], og_demand_scheduled[NUM_CPUS];
  ...
  void schedule(PACKET_QUEUE *queue), process(PACKET_QUEUE *queue), update_schedule_cycle(PACKET_QUEUE *queue), update_process_cycle(PACKET_QUEUE *queue),
      reset_remain_requests(PACKET_QUEUE *queue, uint32_t channel);

  int find_next_request(PACKET_QUEUE *queue, uint8_t &row_buffer_hit), schedule_FR_FCFS(PACKET_QUEUE *queue, uint8_t &row_buffer_hit),
      schedule_FRONTAL_FR_FCFS(PACKET_QUEUE *queue, uint8_t &row_buffer_hit), schedule_FR_FRONTAL_FCFS(PACKET_QUEUE *queue, uint8_t &row_buffer_hit),
      schedule_ROB_PART_FR_FCFS(PACKET_QUEUE *queue, uint8_t &row_buffer_hit), schedule_FR_ROB_PART_FCFS(PACKET_QUEUE *queue, uint8_t &row_buffer_hit),
  ...
  uint32_t dram_get_channel(uint64_t address), dram_get_rank(uint64_t address), dram_get_bank(uint64_t address), dram_get_row(uint64_t address),
      dram_get_column(uint64_t address), drc_check_hit(uint64_t address, uint32_t cpu, uint32_t channel, uint32_t rank, uint32_t bank, uint32_t row);
  ...
  // ZLANG: Oogway
  uint32_t get_bw(uint32_t core_id), get_overall_bw(uint32_t core_id), get_pf_bw(uint32_t core_id), get_ocp_bw(uint32_t core_id),
      get_demand_bw(uint32_t core_id), get_bw_needed(uint32_t core_id);

  uint32_t get_total_scheduled(uint32_t core_id) {
    return og_total_scheduled[core_id];
  };

  uint32_t get_pf_scheduled(uint32_t core_id) {
    return og_pf_scheduled[core_id];
  };

  uint32_t get_ocp_scheduled(uint32_t core_id) {
    return og_ocp_scheduled[core_id];
  };
  uint32_t get_demand_scheduled(uint32_t core_id) {
    return og_demand_scheduled[core_id];
  }

  void reset_og_scheduled(uint32_t core_id) {
    og_total_scheduled[core_id] = 0;
    og_pf_scheduled[core_id] = 0;
    og_ocp_scheduled[core_id] = 0;
    og_demand_scheduled[core_id] = 0;
  }
};
