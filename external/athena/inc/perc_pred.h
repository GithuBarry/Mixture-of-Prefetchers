...
typedef enum {
  ...
  LastNLoadPCs,       // 16
  LastNPCs,           // 17
  FLPPrediction,      // 18 - FLP's off-chip prediction
...
struct state_info_t {
  ...
  uint32_t flp_prediction; // New field for FLP prediction
...
public:
  perceptron_pred_t(vector<int32_t> _activated_features, vector<int32_t> weight_array_sizes, vector<int32_t> feature_hash_types, float threshold, float max_w,
                    float min_w, float pos_delta, float neg_delta, float pos_thresh, float neg_thresh);
  ~perceptron_pred_t();

  void set_cpu(int _cpu) {
    cpu = _cpu;
  }
  int get_cpu() {
    return cpu;
  ...
  inline float get_activation_threshold() {
