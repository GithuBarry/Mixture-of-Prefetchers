...
BertiL1::BertiL1(std::string type, CACHE *cache) : Prefetcher(type), m_parent_cache(cache) {
  init_stats();
...
void BertiL1::init_stats() {
  stats.prefetches_issued = 0;
  stats.prefetches_useful = 0;
  stats.prefetches_useless = 0;
}

void BertiL1::print_config() {
  cout << "berti_L1_prefetcher" << endl;
}

void BertiL1::dump_stats() {
  cout << "berti_prefetches_issued " << stats.prefetches_issued << endl
       << "berti_prefetches_useful " << stats.prefetches_useful << endl
       << "berti_prefetches_useless " << stats.prefetches_useless << endl;
}

// TIME AND OVERFLOWS

uint64_t BertiL1::l1d_get_latency(uint64_t cycle, uint64_t cycle_prev) {
...
int BertiL1::l1d_calculate_stride(uint64_t prev_offset, uint64_t current_offset) {
  int stride;
...
void BertiL1::l1d_init_current_pages_table() {
...
uint64_t BertiL1::l1d_get_current_pages_entry(uint64_t page_addr) {
  for (int i = 0; i < L1D_CURRENT_PAGES_TABLE_ENTRIES; i++) {
...
void BertiL1::l1d_update_lru_current_pages_table(uint64_t index) {
...
uint64_t BertiL1::l1d_get_lru_current_pages_entry() {
...
int BertiL1::l1d_get_berti_current_pages_table(uint64_t index, uint64_t &ctr) {
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
  ...
  for (int i = 0; i < L1D_CURRENT_PAGES_TABLE_NUM_BERTI; i++) {
    ...
    if (score > max_score) {
      berti = l1d_current_pages_table[index].berti[i];
      max_score = score;
...
void BertiL1::l1d_add_current_pages_table(uint64_t index, uint64_t page_addr, uint64_t ip, uint64_t offset) {
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
...
uint64_t BertiL1::l1d_update_demand_current_pages_table(uint64_t index, uint64_t offset) {
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
  l1d_current_pages_table[index].u_vector |= (uint64_t)1 << offset;
...
void BertiL1::l1d_add_berti_current_pages_table(uint64_t index, int berti) {
  assert(berti != 0);
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
  for (int i = 0; i < L1D_CURRENT_PAGES_TABLE_NUM_BERTI; i++) {
    if (l1d_current_pages_table[index].berti_ctr[i] == 0) {
      l1d_current_pages_table[index].berti[i] = berti;
      l1d_current_pages_table[index].berti_ctr[i] = 1;
      break;
    } else if (l1d_current_pages_table[index].berti[i] == berti) {
...
bool BertiL1::l1d_requested_offset_current_pages_table(uint64_t index, uint64_t offset) {
  assert(index < L1D_CURRENT_PAGES_TABLE_ENTRIES);
  return l1d_current_pages_table[index].u_vector & ((uint64_t)1 << offset);
}

void BertiL1::l1d_remove_current_table_entry(uint64_t index) {
...
void BertiL1::l1d_init_prev_requests_table() {
...
uint64_t BertiL1::l1d_find_prev_request_entry(uint64_t pointer, uint64_t offset) {
  for (int i = 0; i < L1D_PREV_REQUESTS_TABLE_ENTRIES; i++) {
...
void BertiL1::l1d_add_prev_requests_table(uint64_t pointer, uint64_t offset, uint64_t cycle) {
...
void BertiL1::l1d_reset_pointer_prev_requests(uint64_t pointer) {
  for (int i = 0; i < L1D_PREV_REQUESTS_TABLE_ENTRIES; i++) {
...
uint64_t BertiL1::l1d_get_latency_prev_requests_table(uint64_t pointer, uint64_t offset, uint64_t cycle) {
...
void BertiL1::l1d_get_berti_prev_requests_table(uint64_t pointer, uint64_t offset, uint64_t cycle, int *berti) {
  int my_pos = 0;
  ...
  for (uint64_t i = (l1d_prev_requests_table_head + L1D_PREV_REQUESTS_TABLE_MASK) & L1D_PREV_REQUESTS_TABLE_MASK; i != l1d_prev_requests_table_head;
    ...
    if (l1d_prev_requests_table[i].page_addr_pointer == pointer) {
      if (l1d_prev_requests_table[i].time <= (cycle & L1D_TIME_MASK) + extra_time) {
        berti[my_pos] = l1d_calculate_stride(l1d_prev_requests_table[i].offset, offset);
        my_pos++;
...
void BertiL1::l1d_init_prev_prefetches_table() {
...
uint64_t BertiL1::l1d_find_prev_prefetch_entry(uint64_t pointer, uint64_t offset) {
  for (int i = 0; i < L1D_PREV_PREFETCHES_TABLE_ENTRIES; i++) {
...
void BertiL1::l1d_add_prev_prefetches_table(uint64_t pointer, uint64_t offset, uint64_t cycle) {
  // First find for coalescing
...
void BertiL1::l1d_reset_pointer_prev_prefetches(uint64_t pointer) {
  for (int i = 0; i < L1D_PREV_PREFETCHES_TABLE_ENTRIES; i++) {
...
void BertiL1::l1d_reset_entry_prev_prefetches_table(uint64_t pointer, uint64_t offset) {
...
uint64_t BertiL1::l1d_get_and_set_latency_prev_prefetches_table(uint64_t pointer, uint64_t offset, uint64_t cycle) {
  uint64_t index = l1d_find_prev_prefetch_entry(pointer, offset);
  if (index == L1D_PREV_PREFETCHES_TABLE_ENTRIES) {
    return 0;
  }
...
uint64_t BertiL1::l1d_get_latency_prev_prefetches_table(uint64_t pointer, uint64_t offset) {
  uint64_t index = l1d_find_prev_prefetch_entry(pointer, offset);
  if (index == L1D_PREV_PREFETCHES_TABLE_ENTRIES) {
    return 0;
  }
...
void BertiL1::l1d_init_record_pages_table() {
...
uint64_t BertiL1::l1d_get_lru_record_pages_entry() {
...
void BertiL1::l1d_update_lru_record_pages_table(uint64_t index) {
...
void BertiL1::l1d_add_record_pages_table(uint64_t index, uint64_t page_addr, uint64_t vector, uint64_t first_offset, int berti) {
  assert(index < L1D_RECORD_PAGES_TABLE_ENTRIES);
...
uint64_t BertiL1::l1d_get_entry_record_pages_table(uint64_t page_addr, uint64_t first_offset) {
...
uint64_t BertiL1::l1d_get_entry_record_pages_table(uint64_t page_addr) {
  uint64_t trunc_page_addr = page_addr & L1D_TRUNCATED_PAGE_ADDR_MASK;
  for (int i = 0; i < L1D_RECORD_PAGES_TABLE_ENTRIES; i++) {
...
void BertiL1::l1d_copy_entries_record_pages_table(uint64_t index_from, uint64_t index_to) {
...
void BertiL1::l1d_init_ip_table() {
...
void BertiL1::l1d_record_current_page(uint64_t index_current) {
...
void BertiL1::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit, uint8_t type, std::vector<uint64_t> &pref_addr) {
  ...
  if (index == L1D_CURRENT_PAGES_TABLE_ENTRIES || !l1d_requested_offset_current_pages_table(index, offset)) {
    ...
    if (index < L1D_CURRENT_PAGES_TABLE_ENTRIES) { // Found
      ...
      if (cache_hit) {
        ...
        if (pref_latency != 0) {
          ...
          l1d_get_berti_prev_requests_table(index, offset, current_core_cycle[m_parent_cache->cpu] - pref_latency, berti);
    ...
    l1d_add_prev_requests_table(index, offset, current_core_cycle[m_parent_cache->cpu]);
    ...
    if (pgo_pointer != L1D_RECORD_PAGES_TABLE_ENTRIES &&
      ...
      berti = l1d_record_pages_table[pgo_pointer].berti;
      ...
      if (l1d_record_pages_table[ip_pointer].first_offset == first_offset &&
        ...
        berti = l1d_record_pages_table[ip_pointer].berti;
        match_confidence = 1; // High confidence
        recorded = true;
      } else
        ...
        if (current_berti != 0 && berti_confidence >= L1D_BERTI_CTR_MED_HIGH_CONFIDENCE) { // Medium-High confidence
          ...
          if (pg_pointer != L1D_RECORD_PAGES_TABLE_ENTRIES) { // Medium confidence
            ...
            berti = l1d_record_pages_table[pg_pointer].berti;
            recorded = true;
          } else
        ...
        if (berti > 0) {
          for (uint64_t i = first_burst; i < offset + berti; i++) {
            ...
            if ((((uint64_t)1 << i) & u_vector) && !l1d_requested_offset_current_pages_table(index, pf_offset)) {
              if (pref_addr.size() < L1D_MAX_NUM_BURST_PREFETCHES) {
                pref_addr.push_back(pf_addr);
                l1d_add_prev_prefetches_table(index, pf_offset, current_core_cycle[m_parent_cache->cpu]);
                bursts++;
...
void BertiL1::cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in) {
  uint64_t line_addr = (addr >> LOG2_BLOCK_SIZE);
  uint64_t page_addr = line_addr >> L1D_PAGE_BLOCKS_BITS;
  uint64_t offset = line_addr & L1D_PAGE_OFFSET_MASK;
  ...
  if (pointer_prev < L1D_CURRENT_PAGES_TABLE_ENTRIES) { // Not found, not entry in prev requests
    uint64_t pref_latency = l1d_get_and_set_latency_prev_prefetches_table(pointer_prev, offset, current_core_cycle[m_parent_cache->cpu]);
    uint64_t demand_latency = l1d_get_latency_prev_requests_table(pointer_prev, offset, current_core_cycle[m_parent_cache->cpu]);
    ...
    if (demand_latency != 0) { // Not found, berti will not be found neither
      ...
      int berti[L1D_CURRENT_PAGES_TABLE_NUM_BERTI_PER_ACCESS];
      l1d_get_berti_prev_requests_table(pointer_prev, offset, current_core_cycle[m_parent_cache->cpu] - (pref_latency + demand_latency), berti);
      for (int i = 0; i < L1D_CURRENT_PAGES_TABLE_NUM_BERTI_PER_ACCESS; i++) {
        if (berti[i] == 0) {
          break;
        }
