...
LearningEngineBasic::LearningEngineBasic(Prefetcher *parent, float alpha, float gamma, float epsilon, uint32_t actions, uint32_t states, uint64_t seed,
...
void LearningEngineBasic::plot_scores() {
  ...
  for (uint32_t index = 0; index < knob::le_plot_actions.size(); ++index) {
    ...
    fprintf(script, "'%s' using 1:%u with lines title \"action_index[%u]\"", knob::le_trace_file_name.c_str(), (knob::le_plot_actions[index] + 2),
