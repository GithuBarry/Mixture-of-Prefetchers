...
FeatureKnowledge::FeatureKnowledge(FeatureType feature_type, float alpha, float gamma, uint32_t actions, float weight, float weight_gradient,
  ...
  assert(m_num_tilings == 1 || m_enable_tiling_offset); /* enforce the use of tiling offsets in case
                                                           of multiple tilings */
...
void FeatureKnowledge::updateQ(State *state1, uint32_t action1, int32_t reward, State *state2, uint32_t action2) {
  ...
  for (uint32_t tiling = 0; tiling < m_num_tilings; ++tiling) {
    ...
    MYLOG("<tiling %u> Q(%s,%u) = %0.2f, R = %d, Q(%s,%u) = %0.2f, Q(%s,%u) = "
          "%0.2f",
          tiling, state1->to_string().c_str(), action1, Qsa1_old, reward, state2->to_string().c_str(), action2, Qsa2, state1->to_string().c_str(), action1,
          Qsa1);
  }
