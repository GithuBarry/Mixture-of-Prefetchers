...
void StridePrefetcher::print_config() {
  cout << "stride_num_trackers " << knob::stride_num_trackers << endl << "stride_pref_degree " << knob::stride_pref_degree << endl;
}
...
void StridePrefetcher::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, vector<uint64_t> &pref_addr) {
  ...
  // uint32_t offset = (address >> LOG2_BLOCK_SIZE) & ((1ull << (LOG2_PAGE_SIZE
  // - LOG2_BLOCK_SIZE)) - 1);
