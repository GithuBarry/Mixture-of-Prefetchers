...
void IPCP_L1::check_for_stream_l1(int index, uint64_t cl_addr, uint8_t cpu) {
  ...
  if (count > NUM_GHB_ENTRIES / 2) { // stream is detected
    ...
    if (count >= (NUM_GHB_ENTRIES * 3) / 4) { // stream is classified as strong if
                                              // more than 3/4th entries belong to
                                              // stream
      trackers_l1[cpu][index].str_strength = 1;
    }
  } else {
    if (trackers_l1[cpu][index].str_strength == 0) { // if identified as weak
                                                     // stream, we need to reset
...
void IPCP_L1::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit, uint8_t type, std::vector<uint64_t> &pref_addr) {
  ...
  if (trackers_l1[cpu][index].ip_tag != ip_tag) { // new/conflict IP
    if (trackers_l1[cpu][index].ip_valid == 0) {  // if valid bit is zero, update
                                                  // with latest IP info
    ...
    prefetch_line_wrapper(ip, addr, pf_address, FILL_L1, metadata);
    return;
  ...
  if (trackers_l1[cpu][index].str_valid == 1) { // stream IP
  ...
  } else if (DPT_l1[cpu][signature].conf >= 0 && DPT_l1[cpu][signature].delta != 0) { // if conf>=0, continue
                                                                                      // looking for delta
    ...
    for (i = 0; i < prefetch_degree; i++) {
      ...
      if (DPT_l1[cpu][signature].conf > 0) { // prefetch only when conf>0 for
                                             // CPLX
        prefetch_line_wrapper(ip, addr, pf_address, FILL_L1, metadata);
        num_prefs++;
...
void IPCP_L1::prefetch_line_wrapper(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata) {
  if (!knob::og_enable || ooo_cpu[m_parent_cache->cpu].oogway->is_prefetch_enabled(id)) {
    m_parent_cache->prefetch_line(ip, base_addr, pf_addr, pf_fill_level, prefetch_metadata);
    if (ptracker) {
      ptracker->track_pref(pf_addr);
    }
    // printf("IPCP_L1: prefetching %lx\n", pf_addr);
