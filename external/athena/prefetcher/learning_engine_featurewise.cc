...
LearningEngineFeaturewise::LearningEngineFeaturewise(Prefetcher *parent, float alpha, float gamma, float epsilon, uint32_t actions, uint64_t seed,
  ...
  for (uint32_t index = 0; index < knob::le_featurewise_active_features.size(); ++index) {
    ...
    m_feature_knowledges[knob::le_featurewise_active_features[index]] =
        new FeatureKnowledge((FeatureType)knob::le_featurewise_active_features[index], alpha, gamma, actions, knob::le_featurewise_feature_weights[index],
                             knob::le_featurewise_weight_gradient, knob::le_featurewise_num_tilings[index], knob::le_featurewise_num_tiles[index], zero_init,
                             knob::le_featurewise_hash_types[index], knob::le_featurewise_enable_tiling_offset[index]);
...
/* consensus stats: whether each feature's maxAction decision aligns with the
...
bool LearningEngineFeaturewise::do_fallback(State *state) {
  /* set fallback to whatever the knob value is, if dynamic fallback is turned
   * off */
  if (!knob::le_featurewise_enable_dyn_action_fallback) {
