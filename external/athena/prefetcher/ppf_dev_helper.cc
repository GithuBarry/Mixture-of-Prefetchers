...
void PATTERN_TABLE::update_pattern(uint32_t last_sig, int curr_delta) {
  ...
  if (match == PT_WAY) {
    ...
    for (match = 0; match < PT_WAY; match++) {
      if (c_delta[set][match] < min_counter) { // Select an entry with the
                                               // minimum c_delta
        victim_way = match;
  ...
  if (c_sig[set]) {
    for (uint32_t way = 0; way < PT_WAY; way++) {
      ...
      if (pf_conf && do_pf && pf_q_tail < 100) {
        ...
        // cout << "WAY:  "<< way << "\tPF_CONF: " << pf_conf <<  "\tIndex: " <<
        // pf_q_tail << endl;
      ...
      if (pf_conf && pf_q_tail < L2C_MSHR_SIZE && (perc_sum < knob::ppf_perc_threshold_hi)) {
        // Note: Using knob::ppf_perc_threshold_hi as the decising factor for
        // negative case Because 'trueness' of a prefetch is decisded based on
        // the feedback from L2C So even though LLC prefetches go through, they
        // are treated as false wrt L2C in this case
...
bool PREFETCH_FILTER::check(uint64_t check_addr, uint64_t base_addr, uint64_t ip, FILTER_REQUEST filter_request, int cur_delta, uint32_t last_sig,
  ...
  case SPP_PERC_REJECT: // To see what would have been the prediction given
    ...
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      ...
      // rejected So even in perc reject case, I'm checking in the accept
  ...
  case SPP_LLC_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      ...
      // Therefore, it is safe to prefetch this cache line in the large LLC
      // and save precious L2C capacity If this prefetch request becomes more
      // confident and SPP eventually issues SPP_L2C_PREFETCH, we can get this
      // cache line immediately from the LLC (not from DRAM) To allow this
      // fast prefetch from LLC, SPP does not set the valid bit for
      // SPP_LLC_PREFETCH
  ...
  case L2C_DEMAND:
    if ((remainder_tag[quotient] == remainder) && (useful[quotient] == 0)) {
      useful[quotient] = 1;
      if (valid[quotient]) {
        ghr->pf_useful++; // This cache line was prefetched by SPP and
    ...
    if (!(valid[quotient] && remainder_tag[quotient] == remainder)) {
      ...
      if (valid_reject[quotient_reject] && remainder_tag_reject[quotient_reject] == remainder_reject) {
        SPP_DP(cout << "[FILTER] " << __func__ << " not doing anything for check_addr: " << hex << check_addr << " cache_line: " << cache_line << dec;
               ...
               cout << " ghr->pf_issued: " << ghr->pf_issued << " ghr->pf_useful: " << ghr->pf_useful << endl;
               cout << " Calling Perceptron Update (DEC) as a useful "
...
int32_t PERCEPTRON::perc_predict(uint64_t base_addr, uint64_t ip, uint64_t ip_1, uint64_t ip_2, uint64_t ip_3, int32_t cur_delta, uint32_t last_sig,
  ...
  SPP_DP(cout << " Sum of perceptrons: " << sum
              << " Prediction made: " << ((sum >= knob::ppf_perc_threshold_lo) ? ((sum >= knob::ppf_perc_threshold_hi) ? FILL_L2 : FILL_LLC) : 0) << endl;);
  // Return the sum
  return sum;
}
...
void PERCEPTRON::perc_update(uint64_t base_addr, uint64_t ip, uint64_t ip_1, uint64_t ip_2, uint64_t ip_3, int32_t cur_delta, uint32_t last_sig,
  ...
  if (!direction) { // direction = 1 means the sum was in the correct direction,
    ...
    SPP_DP(int differential = (sum >= knob::ppf_perc_threshold_hi) ? -1 : 1; cout << " Direction is: " << direction << " and sum is:" << sum;
  ...
  if (direction && sum > NEG_UPDT_THRESHOLD && sum < POS_UPDT_THRESHOLD) {
    ...
    SPP_DP(int differential = 0; if (sum >= knob::ppf_perc_threshold_hi) differential = 1; if (sum < knob::ppf_perc_threshold_hi) differential = -1;
