...
Bingo::Bingo(string type, CACHE *cache)
    : Prefetcher(type), parent(cache), pattern_len(knob::bingo_pattern_len), filter_table(knob::bingo_ft_size, knob::bingo_debug_level),
      ...
      pht(knob::bingo_pht_size, knob::bingo_pattern_len, knob::bingo_min_addr_width, knob::bingo_max_addr_width, knob::bingo_pc_width, knob::bingo_debug_level,
          knob::bingo_pht_ways),
      pf_streamer(nullptr, knob::bingo_pf_streamer_size, knob::bingo_pattern_len, knob::bingo_debug_level), debug_level(knob::bingo_debug_level) {
  this->create_ptracker(knob::ptracker_num_sets, knob::ptracker_num_ways);
  this->pf_streamer = PrefetchStreamer(this->ptracker, knob::bingo_pf_streamer_size, knob::bingo_pattern_len, knob::bingo_debug_level);
  init_knobs();
  init_stats();
...
int Bingo::prefetch(uint64_t block_number) {
  int pf_issued = this->pf_streamer.prefetch(id, parent, block_number);
  if (this->debug_level >= 2) {
 ...
 * @return The appropriate prefetch level for all blocks based on PHT output or
