...
old `MoPLite` but still negative versus pair-best single.

## Frozen Boundary

The Stage 2 candidate seed is `MoPLiteGuarded`, an explicit router variant
...
- baseline router: existing `MoPLite` with `mop_router_type=4`
- candidate router: `MoPLiteGuarded` with `mop_router_type=5`
...
- secondary comparators: no-prefetch, old `MoPLite`, and `AthenaMAB`
...
`MoPLiteGuarded` keeps the Stage 1 score signal but changes two structural
...
- two-trace smoke run: `MoPLiteGuarded` reached `1.0205x` vs no-prefetch, old
  `MoPLite` was approximately neutral, and `SPP+PPF` remained the strongest
  single-expert reference.
- 13-local-trace 1M scout: `MoPLiteGuarded` improved old `MoPLite` from
...
- official 10-trace search subset at `5M` warmup + `10M` simulation:
  `MoPLiteGuarded` reached `1.004932x` vs no-prefetch, old `MoPLite` reached
...
- presenting `MoPLiteGuarded` as a win without pair-best-single comparison
