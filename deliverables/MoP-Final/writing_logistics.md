...
Plots use a stable semantic palette. OpenEvolve router results use `#dc267f`. Manual router results use `#fe6100`. Constituent prefetchers use `#648fff` for the main expert hue, with opacity separating Expert 1 and Expert 2 in per-trace plots. The best expert uses `#3f6fd1`, a darker blue reference. Disabled prefetching uses black. Light grey is reserved for gridlines and range guides. Model-search plots use the OpenEvolve-adjacent family `#dc267f`, `#785ef0`, and `#648fff` because all three curves are generated by OpenEvolve runs.
...
- `scripts/make_figures.py` was removed after `scripts/make_stage2_final_assets.py` became the maintained figure and table builder.
- Historical pre-OpenEvolve tables moved to `report/tables/legacy_stage1/` so the main table folder only contains current report tables.
